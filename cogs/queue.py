...
import discord
from discord import Embed, Option, FFmpegPCMAudio
...
class Queue(commands.Cog):
    ...
    def error_playing_song(self, e, ctx: discord.ApplicationContext):
        if e:
            ...
            asyncio.run_coroutine_threadsafe(ctx.channel.send(
                f"There was an error playing {data[ctx.guild.id]['songs'][0]['title']}. Skipping"), self.bot.loop)
    ...
    def added_to_queue_embed(self, title, webpage_url, thumbnail_url, uploader):
        ...
        embed.add_field(
            name=uploader, value=f"[{title}]({webpage_url})", inline=False)
        return embed
    ...
    async def play_song(self, ctx: discord.ApplicationContext):
        ...
        if len(data[ctx.guild.id]['songs']) > 0:
            ...
            # streamable youtube url by FFMPEG
            source = data[ctx.guild.id]['songs'][0]['source']
            # thumbnail url
            thumbnail_url = data[ctx.guild.id]['songs'][0]['thumbnail_url']
            ...
            ctx.guild.voice_client.play(FFmpegPCMAudio(
                source, **self.FFMPEG_OPTIONS), after=lambda e: self.error_playing_song(e, ctx))

            while ctx.guild.voice_client.is_playing() or ctx.guild.voice_client.is_paused():
                # wait for 2 seconds before checking to avoid playing the next song too fast
                await asyncio.sleep(2)

            # do we loop the song?
            loop = data[ctx.guild.id]['loop_current_music']
    ...
    async def queue(self, ctx: discord.ApplicationContext):
        ...
        if len(songs_list) > 1:  # ['current song', 'next song']
            ...
            embed = Embed(
                color=0x44a8de, title="New Queue" if self.is_shuffled else "Queue", description=Embed.Empty)
            self.is_shuffled = False
            embed.add_field(name="Up Next:", value=f"1 song remaining")
            embed.add_field(
                name=f"[1] - {next_song['title']}", value=next_song['uploader'], inline=False)
        ...
        # ['current song', 'next song', '2nd next song', '3rd next song']
        if len(songs_list) > 2:
            embed.set_field_at(index=0, name="Up Next:",
                               value=f"{len(songs_list[1:])} songs remaining")
            n = 2
            for song in songs_list[2:]:
                if n < 12:
                    embed.add_field(
                        name=f"[{n}] {song['title']}", value=song['uploader'], inline=False)
                else:
                    if n > 12:
                        embed.set_footer(
                            text=f"{len(songs_list)[10:]} more songs in the queue", icon_url=Embed.Empty)
                    break
                n += 1
